...
let calScript = '!function(e,t){var n=e.documentElement,i="orientationchange"in window?"orientationchange":"resize",d=function(){var e=n.clientWidth;n.style.fontSize=e/baseWidth*remUnit+"px",n.setAttribute("data-dpr",Math.floor(window.devicePixelRatio))};d(),e.addEventListener&&t.addEventListener(i,d,!1)}(document,window);';
      ...
      options.templates.forEach((template) => {
        ...
        if (htmlAsset) {
          ...
          htmlAsset.source = function () {
            calScript = calScript.replace('baseWidth', options.baseWidth);
            calScript = calScript.replace('remUnit', options.remUnit);
            // 注入动态计算脚本
            htmlContent = htmlContent.replace('<head>', `<head><script>${calScript}</script>`);
            return htmlContent;
          }
        }
      })
